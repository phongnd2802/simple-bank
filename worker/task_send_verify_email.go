...
// ProcessTaskVerifyEmail implements TaskProcessor.
func (processor *RedisTaskProcessor) ProcessTaskVerifyEmail(ctx context.Context, task *asynq.Task) error {
	...
	verifyUrl := fmt.Sprintf("http://localhost:8002/api/v1/verify-email?email_id=%d&secret_code=%s", verifyEmail.ID, verifyEmail.SecretCode)
	...
	log.Info().Str("type", task.Type()).Bytes("payload", task.Payload()).
		Str("email", user.Email).Msg("processted task")
